...
class VouchRequest(db.Base, PostgresSerializerMixin):
    ...
    def find_by_top_id(top_id, page=0, pagesize=10):
        return db.session.query(VouchRequest).filter_by(top_id = top_id).limit(pagesize).offset(pagesize * page).all()

    @staticmethod
    def find_by_bottom_id(bottom_id, page=0, pagesize=10):
        return db.session.query(VouchRequest).filter_by(bottom_id = bottom_id).limit(pagesize).offset(pagesize * page).all()
